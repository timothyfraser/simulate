...
#' @param data Data.frame containing multivariate normal distribution of coefficient terms, generated by equate(). 
#' @param ci (Optional) Single value from 0 to 1. Defaults to 0.95, for a 95% confidence interval.
#' @param mu (default = 0) To compute statistical significance of estimates, specify the null hypothesis (eg. estimate = 0). mu = 0 is most common.
...
#'   \item{p_value}{p-value calculated from simulations. Depicts false positive rate as the percentage of simulations on opposite side of 0 (mu) from the observed coefficient.}
...
#'   get_coeftable()
#'   
#' # More recommended is to use the tabulate() function, 
...
get_coeftable = function(data, ci = 0.95, mu = 0){
  require(dplyr)
  require(tibble)
  require(gtools)
  
  if(is.null(data)){
    print("Please provide a multivariate normal distribution of equations, outputted by equate().")
    stop()
  }
  ...
  data %>%
    ...
    summarize(
      ...
      se = sd(simcoef),
      # Get one-tailed p-value based directly on simulations
      p_value = case_when(
        # Get type-two error rate
        # Get percentage of simulations on opposite side of mu from estimate
        # If estimate OVER mu, then get % of simulations LESS than or EQUAL to mu
        estimate > mu ~ mean(simcoef <= mu),
        # If estimate UNDER mu, then get % of simulations GREATER than or EQUAL to mu
        estimate < mu ~ mean(simcoef >= mu)),
