...
#'   \item{p_value}{p-value calculated from simulations (one-tailed). Depicts false positive rate as the percentage of simulations on opposite side of 0 from the observed coefficient.}
...
get_coeftable = function(data, ci = 0.95){
  ...
  data %>%
    ...
    summarize(
      ...
      # Calculating the type-two error rate, meaning
      # percentage of simulations on opposite side 
      # of zero from estime
      p_value = case_when(
        # if estimate is over zero,
        # what percentage of sims are LESS than or EQUAL to ZERO?
        estimate > 0 ~ mean(simcoef <= 0),
        # if estimate is less than zero,
        # what percentage of sims are GREATER than or EQUAL to ZERO?
        estimate < 0 ~ mean(simcoef >= 0)), 
